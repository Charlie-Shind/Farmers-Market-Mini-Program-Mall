...
export class PlatformDataService {
  ...
  private getWechatMiniProgramCredentials(): { appId: string; secret: string } {
    ...
    if (!appId || !secret) {
      throw new InternalServerErrorException('Phone verification is unavailable');
    }
  ...
  private getWechatIdentityCredentials(): { appId: string; secret: string } {
    ...
    if (!appId || !secret) {
      throw new InternalServerErrorException('SMS login is unavailable');
    }
  ...
  async bindWechatPhone(body: Record<string, unknown>) {
    ...
    const mobile = await this.resolveWechatPhoneNumber(phoneCode);
    const openid = `wechat_${loginCode}`;
  ...
  async createWechatPayment(authUser: AuthUser, body: Record<string, unknown>) {
    ...
    return {
      appId: this.getWechatMiniProgramCredentials().appId,
      paymentNo: payNo,
      orderNo,
