...
function computeThemeFromLuminance(luminance: number, threshold: number): ThemeResolveResult {
...
Component({
  ...
  methods: {
    ...
    async resolveTheme() {
      const seq = ++detectSeq;
      const { mode, imageUrl, threshold } = this.data as {
        mode: ThemeMode;
        imageUrl: string;
        threshold: number;
      };
