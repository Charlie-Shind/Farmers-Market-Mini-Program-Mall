...
import {
  ...
  fetchMerchantProductDraft,
} from '../../../../services/merchant';
import { fetchCategories, AppCategory } from '../../../../services/app';
import { buildPageTopStyle } from '../../../../utils/page-layout';
import { ensureMerchantAccess, navigateBackOrMerchantHome } from '../../../../utils/auth-route';
...
const PRE_SALE_FIELDS = ['isPreSale', 'offlinePrice', 'safetyStock'] as const;
...
const createSpecId = (prefix: string) =>
  `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

const createDefaultSpecGroup = (): SpecGroup => ({
...
function normalizeEditAnchor(value?: string): string {
...
function buildSpecGroupsFromRecord(
  specJson: Record<string, string>,
  fallbackName: string,
): SpecGroup[] {
...
function buildGeneratedSkuRows(
  groups: SpecGroup[],
  defaults: Pick<GeneratedSkuRow, 'price' | 'originalPrice' | 'stock' | 'skuImageUrl'>,
  preservedRows: GeneratedSkuRow[] = [],
): GeneratedSkuRow[] {
  const activeGroups = groups.filter(
    (group) => group.name.trim() && group.options.some((option) => option.label.trim()),
  );
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      skuName:
        preserved?.skuName || row.labelParts.join(' / ') || '规格项',
      specJson: row.specJson,
      price: preserved?.price || defaults.price,
      originalPrice: preserved?.originalPrice || defaults.originalPrice,
      stock: preserved?.stock || defaults.stock,
      skuImageUrl: preserved?.skuImageUrl || defaults.skuImageUrl,
    };
  });
}

const PRODUCT_SERVICE_OPTIONS: ProductServiceOption[] = [
  {
    key: 'badFruit',
    title: '坏果包赔',
    desc: '坏果售后保障',
...
Component({
  data: {
    ...
    selectedCategory: null as AppCategory | null,
    pendingCategoryParent: null as AppCategory | null,
    showMoreInfo: false,
    moreInfoFilledCount: 0,
    isMultiSpec: false,
    specGroups: [] as SpecGroup[],
    generatedSkuRows: [] as GeneratedSkuRow[],
    form: {
      title: '',
      subtitle: '',
      coverUrl: '',
      ...
      videoUrl: '',
      videoCover: '',
      serviceTagKeys: [] as string[],
      isPreSale: false,
      offlinePrice: '',
      safetyStock: '',
      soldCountText: '',
    } as FormFields & { soldCountText: string },
    draftRestored: false,
  },
  ...
  lifetimes: {
    attached() {
      ...
      this.bootstrap(current?.options || {});
    },
  },
  ...
  methods: {
    bootstrap(query: Record<string, string>) {
      if (
        !ensureMerchantAccess(
          `/pages/merchant/products/edit/edit` + (query.productId ? `?productId=${query.productId}` : ''),
        )
      ) {
        return;
      }
    ...
    toggleMultiSpec(e: WechatMiniprogram.SwitchChange) {
      ...
      this.setData({
        ...
        generatedSkuRows: [
          {
            key: 'default',
            label: defaultSku?.label || this.data.form.skuName || '规格项',
            skuName: defaultSku?.skuName || this.data.form.skuName || '规格项',
            specJson: {},
            price: this.data.form.price || defaultSku?.price || '0.00',
            originalPrice:
              this.data.form.originalPrice || defaultSku?.originalPrice || this.data.form.price || '0.00',
            stock: this.data.form.stock || defaultSku?.stock || '0',
            skuImageUrl: defaultSku?.skuImageUrl || '',
          },
        ],
      });
    },
    ...
    async loadProductDetails(productId: number) {
      try {
        ...
        const product = await fetchMerchantProductDetail(productId);
        wx.hideLoading();
        ...
        this.setData({
          ...
          form: {
            ...
            images,
            videoUrl: (product.videos && product.videos[0]?.videoUrl) || '',
            videoCover: (product.videos && product.videos[0]?.coverUrl) || '',
            ...
            // 后端补字段后无需改此处。
            soldCountText: this.formatSoldCount(
              (product as { soldCount?: number | string }).soldCount,
            ),
          },
          specGroups,
        });
        ...
        this.setData(
          {
            generatedSkuRows: buildGeneratedSkuRows(
              specGroups,
              {
                price,
                originalPrice,
        ...
        if (product.categoryId && this.data.categories.length > 0) {
          // 优先在二级分类中查找，未命中再回到一级
          let matched: AppCategory | null = null;
          for (const parent of this.data.categories) {
            const child = parent.children?.find((c) => c.id === product.categoryId);
            if (child) {
              matched = {
                id: child.id,
        ...
        setTimeout(() => navigateBackOrMerchantHome(), 800);
      }
    },

    onFieldInput(e: WechatMiniprogram.Input) {
      const field = String((e.currentTarget.dataset as { field?: string }).field || '');
    ...
    onCategoryParentChange(e: WechatMiniprogram.PickerChange) {
    ...
    onCategoryChildChange(e: WechatMiniprogram.PickerChange) {
    ...
    toggleServiceTag(e: WechatMiniprogram.BaseEvent) {
    ...
    onPreSaleChange(e: WechatMiniprogram.SwitchChange) {
      this.setData(
        {
    ...
    recomputeMoreInfoSummary() {
      ...
      let count = 0;
      if (form.serviceTagKeys.length > 0) count += 1;
      if (form.originPlace.trim()) count += 1;
      if (form.traceCode.trim() || form.traceDesc.trim()) count += 1;
      if (form.videoUrl) count += 1;
      if (form.isPreSale) count += 1;
    ...
    toggleSpecGroup(e: WechatMiniprogram.BaseEvent) {
    ...
    onSpecGroupNameInput(e: WechatMiniprogram.Input) {
      const index = Number((e.currentTarget.dataset as { index?: number }).index);
      if (Number.isNaN(index)) return;
      const value = String(e.detail.value ?? '');
      const specGroups = this.data.specGroups.slice();
    ...
    addSpecOption(e: WechatMiniprogram.BaseEvent) {
      const index = Number((e.currentTarget.dataset as { index?: number }).index);
      if (Number.isNaN(index)) return;
      const specGroups = this.data.specGroups.slice();
      const group = specGroups[index];
    ...
    onSpecOptionInput(e: WechatMiniprogram.Input) {
      ...
      if (Number.isNaN(groupIndex) || Number.isNaN(optionIndex)) return;
      const value = String(e.detail.value ?? '');
      const specGroups = this.data.specGroups.slice();
    ...
    removeSpecOption(e: WechatMiniprogram.BaseEvent) {
      const dataset = (e.currentTarget.dataset as {
        groupIndex?: number;
        optionIndex?: number;
      }) || {};
    ...
    onGeneratedSkuInput(e: WechatMiniprogram.Input) {
      const dataset = (e.currentTarget.dataset as { rowIndex?: number; field?: string }) || {};
    ...
    rebuildSkuRows() {
      const rows = buildGeneratedSkuRows(
        this.data.specGroups,
        {
          price: this.data.form.price || '0.00',
          originalPrice: this.data.form.originalPrice || this.data.form.price || '0.00',
    ...
    scrollToAnchorTarget(target: string) {
    ...
    onRemoveBannerImage(e: WechatMiniprogram.BaseEvent) {
    ...
    async probeVideoDomain(url: string) {
      if (!url) return;
      try {
        ...
        const info = await new Promise<{
          ...
          const video = (wx as unknown as {
            createVideoContext?: (id: string, comp?: unknown) => unknown;
          }).createVideoContext;
    ...
    buildPayload(status: 0 | 1, includePreSaleFields = ENABLE_PRE_SALE_FIELDS) {
      ...
      const specJson = isMultiSpec ? skus[0]?.specJson || {} : {};
    ...
    readFeatureFlags(): { preSale: boolean } {
      try {
        const raw = wx.getStorageSync(STORAGE_KEY_FEATURE_FLAGS);
        if (!raw) return { preSale: ENABLE_PRE_SALE_FIELDS };
        const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
        return { preSale: Boolean(parsed?.preSale) };
      } catch (e) {
        return { preSale: ENABLE_PRE_SALE_FIELDS };
      }
    },

    writeFeatureFlag(key: 'preSale', value: boolean) {
      try {
    ...
    async tryRestoreDraft(): Promise<boolean> {
      // 优先尝试云端草稿（若云端存在 draftId），其次本地
      const cloudId = wx.getStorageSync(PRODUCT_DRAFT_KEY + ':cloudId');
      if (cloudId) {
        const cloudDraft = await fetchMerchantProductDraft(cloudId);
        if (cloudDraft && (cloudDraft as { form?: unknown }).form) {
          this.applyDraft(cloudDraft as unknown as { form: FormFields; selectedCategory: AppCategory | null; isMultiSpec: boolean; specGroups: SpecGroup[]; generatedSkuRows: GeneratedSkuRow[] });
    ...
    applyDraft(draft: {
      ...
      generatedSkuRows: GeneratedSkuRow[];
    }) {
      this.setData({
    ...
    shouldExpandMoreInfo(form: Partial<FormFields>): boolean {
      return Boolean(
        form.serviceTagKeys?.length ||
          form.originPlace ||
          form.traceCode ||
          form.traceDesc ||
          form.videoUrl ||
    ...
    async submitAndPublish() {
      ...
      try {
        ...
        try {
          if (productId) {
            await updateMerchantProduct(productId, initialPayload);
          } else {
            await createMerchantProduct(initialPayload);
          }
        } catch (firstErr) {
          ...
          if (flags.preSale && this.isPreSaleError(firstErr)) {
            ...
            if (productId) {
              await updateMerchantProduct(productId, fallbackPayload);
            } else {
              await createMerchantProduct(fallbackPayload);
            }
            wx.showToast({
            ...
            this.clearDraft();
            setTimeout(() => navigateBackOrMerchantHome(), 1000);
            return;
          }
          throw firstErr;
        }
