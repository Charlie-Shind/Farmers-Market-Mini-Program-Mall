...
Page({
    ...
    async loadData() {
        ...
        try {
            const [wallet, records] = await Promise.all([
                ...
                (0, merchant_1.fetchMerchantWithdraws)().catch(() => []),
            ]);
            if (wallet) {
                this.setData({
    ...
    confirmAccount() {
        ...
        const info = typeMap[this.data.accountType] || typeMap.merchant;
    ...
    async submitWithdraw() {
        ...
        try {
            await (0, merchant_1.createMerchantWithdraw)({
                amount,
