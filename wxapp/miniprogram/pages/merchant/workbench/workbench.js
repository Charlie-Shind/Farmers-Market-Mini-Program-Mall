...
Page({
    ...
    onShow() { this.setData({ pageStyle: (0, page_layout_1.buildPageTopStyle)(8) }); this.loadTodos(); },
    async loadTodos() {
        try {
            const wb = await (0, merchant_1.fetchMerchantWorkbench)();
            if (!wb || !wb.todos)
                return;
            const rows = this.data.sections;
            if (rows[0])
                rows[0].rows[0].status = `${wb.todos.lowStock || 0}个预警`;
            if (rows[1]) {
                rows[1].rows[0].status = `${wb.todos.pendingShip || 0}待发货`;
                rows[1].rows[2].status = `${wb.todos.pendingRefund || 0}待处理`;
            }
            if (rows[0])
                rows[0].rows[1].value = String(wb.todos.draftProducts || 0);
