...
Page({
    ...
    loadDrafts() {
        try {
            ...
            if (raw1) {
                const v1Drafts = JSON.parse(raw1);
                allDrafts.push(...v1Drafts.map((d) => (Object.assign(Object.assign({}, d), { _source: 'v1' }))));
            }
            ...
            if (raw2) {
                ...
                if (v2 && v2.form) {
                    allDrafts.push({
                        id: `v2_${Date.now()}`,
                        title: (v2.form && v2.form.title) || '未命名商品',
                        icon: 'edit',
    ...
    applyFilter() {
        const allDrafts = this.data.allDrafts;
        const tab = this.data.filterTab;
        ...
        if (tab === 'noimage') {
            ...
            list = list.filter((d) => {
                const p = getPayload(d);
                return !p.skus || !p.skus.length;
            });
        }
        else if (tab === 'ready') {
            list = list.filter((d) => {
                const p = getPayload(d);
                return p.title && p.price && p.images.length;
            });
        }
        // 搜索
        if (keyword) {
    ...
    onDraftAction(e) {
        ...
        if (action === 'edit') {
            ...
            if ((draft === null || draft === void 0 ? void 0 : draft._source) === 'v2') {
                // v2 草稿: edit 页面会自动 restore
                wx.navigateTo({ url: '/pages/merchant/products/edit/edit' });
            }
            else if ((draft === null || draft === void 0 ? void 0 : draft._source) === 'v1') {
                ...
                const v2 = {
                    ...
                    specGroups: [],
                    generatedSkuRows: Array.isArray(p.skus) ? p.skus.map((s) => ({
                        key: s.skuName || '规格项',
                        label: s.skuName || '规格项',
                ...
                wx.setStorageSync(DRAFTS_V2_KEY, v2);
                wx.navigateTo({ url: '/pages/merchant/products/edit/edit' });
            }
            else {
                wx.navigateTo({ url: '/pages/merchant/products/edit/edit' });
    ...
    async publishDraft(draftId) {
        const allDrafts = this.data.allDrafts.slice();
        ...
        try {
            if (draft._source === 'v2') {
                ...
                const result = await (0, merchant_1.syncMerchantProductDraft)(draft.payload);
                if (result) {
                    const remaining = allDrafts.filter((d) => d.id !== draftId);
                    this.saveDraftsToStorage(remaining);
                    wx.showToast({ title: '已发布', icon: 'success' });
            ...
            success: (res) => {
                if (res.confirm) {
                    ...
                    const remaining = allDrafts.filter((d) => {
                        if (d._source === 'v2') {
                            const f = d.payload && d.payload.form;
                            return f && f.title && f.price;
                        }
                        return d.payload && d.payload.title && d.payload.price;
                    });
                    this.saveDraftsToStorage(remaining);
    ...
    saveDraftsToStorage(drafts) {
        try {
            ...
            if (v2.length > 0) {
                ...
                try {
                    wx.removeStorageSync(DRAFTS_V2_KEY);
                }
                catch (_a) { }
            }
    ...
    applySheetFilter() {
        const completeness = this.data.sheetCompleteness;
