...
import { fetchMerchantOrderDetail, acceptMerchantOrder, type MerchantOrderDetail } from '../../../services/merchant';
...
Page<Record<string, any>, Record<string, any>>({
  ...
  onLoad(options: Record<string, string>) {
    ...
    if (orderNo) { this.setData({ orderNo }); this.loadDetail(orderNo); }
  },
  ...
  async loadDetail(orderNo: string) {
    ...
    try {
      const d: MerchantOrderDetail = await fetchMerchantOrderDetail(orderNo);
      if (!d) { this.setData({ loading: false }); return; }

      const addr = (d.addressSnapshot || {}) as Record<string, any>;
      const item = d.items?.[0] || {};
      const canAccept = !!(d as any).canAccept;
      const canShip = !!(d as any).canShip;
  ...
  copyAddress() {
    const b = this.data.buyer as any;
  ...
  contactBuyer() {
    ...
    wx.showActionSheet({
      ...
      success: (res) => {
        if (res.tapIndex === 0 && b.phone) wx.makePhoneCall({ phoneNumber: b.phone });
        if (res.tapIndex === 1) this.copyAddress();
      },
    });
  },
});
