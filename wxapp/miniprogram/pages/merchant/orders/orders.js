...
Page({
    ...
    onShow() { this.setData({ pageStyle: (0, page_layout_1.buildPageTopStyle)(8) }); this.loadOrders(); },
    async loadOrders() {
        this.setData({ loading: true });
        try {
            const result = await (0, merchant_1.fetchMerchantOrders)({ page: 1, pageSize: 50 });
            const items = (result.items || result || []);
            const orders = items.map((o) => {
                var _a, _b;
                const item = ((_a = o.items) === null || _a === void 0 ? void 0 : _a[0]) || ((_b = o.itemPreview) === null || _b === void 0 ? void 0 : _b[0]) || {};
    ...
    onTabTap(e) {
        ...
        const filterName = activeTab === null || activeTab === void 0 ? void 0 : activeTab.name;
        const orders = tabNames.includes(filterName)
            ? this.data.allOrders.filter((o) => o.status === filterName)
            : this.data.allOrders;
    ...
    onFilterTap(e) {
        ...
        if (index === 3)
            this.setData({ orders: this.data.allOrders.slice(0, 5) });
    },
    openOrderDetail(e) {
        var _a, _b;
        const orderNo = ((_b = (_a = e.currentTarget) === null || _a === void 0 ? void 0 : _a.dataset) === null || _b === void 0 ? void 0 : _b.orderNo) || '';
    ...
    contactBuyer(e) {
        ...
        wx.showActionSheet({
            ...
            success: (res) => {
                ...
                if (res.tapIndex === 1)
                    wx.setClipboardData({ data: 'buyer_wx_2026' });
                if (res.tapIndex === 2)
                    this.openOrderDetail(e);
            },
        });
    },
    handlePrimaryAction(e) {
        ...
        if (action === 'accept')
            this.handleAccept(orderNo);
        else if (action === 'ship')
            wx.navigateTo({ url: `/pages/merchant/logistics/logistics?orderNo=${orderNo}` });
