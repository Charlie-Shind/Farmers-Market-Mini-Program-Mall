...
Page({
    ...
    async loadActivityDetail(id) {
        try {
            const detail = await (0, merchant_1.fetchMerchantActivityDetail)(id);
            if (!detail) return;
            const d = detail;
            const actType = d.activityType || '';
            const typeKey = actType === 'GROUP_BUY' ? 'group' : actType === 'CASHBACK' ? 'coupon' : actType === 'PRESALE' ? 'presell' : 'seckill';
            this.setData({
                activityId: Number(d.id || d.activityId || id),
                activityName: d.title || d.activityName || '',
                currentType: typeKey,
                typeName: TYPE_MAP[typeKey] || '',
    ...
    async saveDraft() {
        ...
        try {
            const selected = this.data.selectedProducts[0];
            await (0, merchant_1.createMerchantActivityDraft)({
    ...
    async saveActivity() {
        ...
        try {
            ...
            if (activityId) {
                await (0, merchant_1.updateMerchantActivity)(activityId, {
                    title: this.data.activityName,
                    startAt: `${this.data.startDate} 00:00`,
                    endAt: `${this.data.endDate} 23:59`,
                    products: [{
                ...
                await (0, merchant_1.createMerchantActivity)({
                    title: this.data.activityName,
                    activityType: resolveActivityType(String(this.data.currentType || 'seckill')),
                    startAt: `${this.data.startDate} 00:00`,
                    endAt: `${this.data.endDate} 23:59`,
